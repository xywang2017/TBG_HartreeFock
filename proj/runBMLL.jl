...
function compute_bmLL(ϕ::Rational,str::String,w0::Float64,w0str::String)
    p = numerator(ϕ)
    q = denominator(ϕ)
    if !isdir(joinpath(fpath,"goldhaber_nostrain/B/data_w$(w0str)/_$(p)_$(q)"))
        mkpath(joinpath(fpath,"goldhaber_nostrain/B/data_w$(w0str)/_$(p)_$(q)"))
    end
    bm = bmLL()
    ...
    fname = joinpath(fpath,"goldhaber_nostrain/B/data_w$(w0str)/_$(p)_$(q)/_$(p)_$(q)_$(str)_metadata.jld2")
    println(fname)
    # params = Params(w1=96.056,w0=w0*96.056,vf=2135.4,dθ=1.05π/180)
    params = Params(ϵ=0.000,Da=0.0,φ=0.0*π/180,dθ=1.38π/180,w1=110,w0=110*w0,vf=2482)
...
# jldopen(joinpath(fpath,"goldhaber_nostrain/B/data_w07/_1_8/_1_8_K_metadata.jld2")) do file 
    # @time begin 
...
function plot_LL_spectrum(ϕs::Vector{Rational{Int}},str::String)
    ...
    for ϕ in ϕs
        ...
        fname = joinpath(fpath,"goldhaber_nostrain/B/data_w$(str)/_$(p)_$(q)/_$(p)_$(q)_K_metadata.jld2")
        ...
        # fname = joinpath(fpath,"goldhaber_nostrain/B/data_w$(str)/_$(p)_$(q)/_$(p)_$(q)_Kprime_metadata.jld2")
        # jldopen(fname) do file 
        #     energies = file["E"][:]
    ...
    # lk = 19
    # bm_path = joinpath(fpath,"goldhaber_nostrain/B0/data/strain2/phi60/bm_lk$(lk).jld2")
