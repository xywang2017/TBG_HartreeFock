...
for st in sts 
    ...
    for ϕ in ϕs 
        ...
        if abs(s+t*p/q) < 4
            metadata = joinpath(fpath,"princeton/B/data_w07/_$(p)_$(q)/1_random_init_HF_$(p)_$(q)_nu_$(νstr).jld2")
            if isfile(metadata)
                E = load(metadata,"iter_energy")[end]
                for flag in ["flavor","random","chern"], seed in 1:6 
                    metadata0 = joinpath(fpath,"princeton/B/data_w$(w0)/_$(p)_$(q)/$(seed)_$(flag)_init_HF_$(p)_$(q)_nu_$(νstr).jld2")
                    if isfile(metadata0)
...
for ϕ in ϕs 
    ...
    for st in sts 
        ...
        if abs(s+t*p/q) < 4
            ...
            E = load(metadata,"iter_energy")[end]
            for flag in ["flavor","random","chern"], seed in 1:6 
                metadata0 = joinpath(fpath,"princeton/B/data_w07/_$(p)_$(q)/$(seed)_$(flag)_init_HF_$(p)_$(q)_nu_$(νstr).jld2")
                if isfile(metadata0)
                    E0 = load(metadata0,"iter_energy")[end]
                    if E0<=E 
                        E, metadata = E0, metadata0 
