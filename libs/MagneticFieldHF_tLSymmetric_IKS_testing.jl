...
function add_HartreeFock(hf::HartreeFock;β::Float64=1.0,φ::Float64=0.0)
    ...
    tmpP = zeros(ComplexF64,2hf.q*hf.nη*hf.ns,2hf.q*hf.nη*hf.ns)
    tmp_tmpP = reshape(tmpP,2hf.q,hf.nη,hf.ns,2hf.q,hf.nη,hf.ns)

    # 
    rps = mod.(collect(1:hf.p:(hf.p*hf.q)).-1,hf.q) .+ 1
        ...
        if abs(G) <G0*cos(pi/6)/abs(cos(mod(angle(G),pi/3)-pi/6)) # this leads to a shell expansion up to 3g1+3g2
            ...
            for ik in 1:size(hf.P,3) 
                ...
                for ip in 1:size(hf.P,3),rp1 in 1:hf.q
                    ...
                    tmp_tmpP[:,1,:,:,2,:] .*= exp(-1im*φ*(rps[rp1]-1))
                    tmp_tmpP[:,2,:,:,1,:] .*= exp(1im*φ*(rps[rp1]-1))
...
function update_P(hf::HartreeFock;Δ::Float64=0.0,_oda::Bool=true,φ::Float64=0.0)
    ...
    for ik in 1:size(hf.H,3)
        ...
        vals[:,ik],vecs[:,:,ik] = eigen(Hermitian(view(hf.H,:,:,ik)) )
