...
function constructbmLL(A::bmLL,params::Params;
                        ϕ::Rational{Int}=1//10,nLL::Int=10,nq::Int=2,fname::String="placeholder.txt", α::Float64=0.3,q0::Complex{Int}=0+0im,
    ...
    A.latt = Lattice() 
    constructLatticeIKS(A.latt,A.params;lk = A.nq*A.q,_valley=_valley,q0=q0)  #[0,1)x[0,1), so far works for p/q < 1
    ...
    if !isempty(fname)
        ...
        ng = 3  # need to set ng = 3 for Hartree Fock calculations; ng=1 needed for quantum geometry
