...
# this version only saves the "necessary" components of the structure factor 
...
mutable struct bmLL
    ...
    Λ::Array{ComplexF64,5}  # nk x mp x ig
    Λmat_idx::Vector{Int} # -(q-1):(q-1), i.e., rp1 - rk1 
...
function constructbmLL(A::bmLL,params::Params;
    ...
    if _calculate_overlap
        A.Λ = zeros(ComplexF64,2A.q*A.nq^2,2A.q*A.nq^2,2A.q-1,size(A.gvec,1),size(A.gvec,2))
        A.Λmat_idx = collect(-(A.q-1):(A.q-1))
...
function computeSpectrum(A::bmLL)
    A.vec = zeros(ComplexF64,2A.nH*A.p,2A.q,A.q,A.nq,A.nq) # inner x 2q x  nkq x nk1
...
function generateU(vec::Array{ComplexF64,5},q::Int,p::Int,nq::Int)
    ...
    rk2 = collect(0:(p-1))
    ips = zeros(Int,q)  # p1 + ip /q 
    for rp1 in 0:(q-1) # p1 + rp1*p/q 
        ips[rp1+1] = mod(rp1 * p,q)
    end
    for rp1 in 1:(q-1)
        vec[:,:,ips[rp1+1]+1,:,:] .= reshape(uvec .* reshape(exp.(-1im*2π*rp1*(rk2./q) ),1,:,1,1,1,1),:,2q,nq,nq)
    end
    return nothing
end
...
function computeCoulombOverlap(A::bmLL,m::Int,n::Int)
    ...
    θstrain = angle(A.params.a2)-π/2
    tmpΛ = reshape(A.Λ,2A.q,A.nq,A.nq,2A.q,2A.q-1,A.nq,A.nq,size(A.gvec,1),size(A.gvec,2))
...
function computeCoulombOverlap_v2(A::bmLL,m::Int,n::Int)
    # !! 2q-1 vs q^2 scaling compared to computeCoulombOverlap() ! much faster!

    # q = p - k + m g1 + n/q * g2, m from -ng to ng, n from -ng*q to ng*q
    # p,k in range [0,1)x[0,1/q), and on a nq x nq grid 
    θstrain = angle(A.params.a2)-π/2
    tmpΛ = reshape(A.Λ,2A.q,A.nq,A.nq,2A.q,A.nq,A.nq,2A.q-1,size(A.gvec,1),size(A.gvec,2))
    ΛΨ = zeros(ComplexF64,2A.nH*A.p,2A.nH*A.p)
    tmpΛΨ = reshape(ΛΨ,A.nH,A.p,2,A.nH,A.p,2)
    Ov = ComplexF64[1 0;0 1]
    for ip2 in 1:A.nq, ik2 in 1:A.nq 
        for ip1 in 1:A.nq, ik1 in 1:A.nq, ipk1 in eachindex(A.Λmat_idx)
            if A.Λmat_idx[ipk1] > 0 
                rp1, rk1 = A.Λmat_idx[ipk1], 0 
            else
                rp1, rk1 = 0, -A.Λmat_idx[ipk1]
            end
            _k1, _p1 = A.latt.k1[ik1+rk1*A.nq], A.latt.k1[ip1+rp1*A.nq]
            for rk2 in 0:(A.p-1)
                rp2, s = mod(rk2+n,A.p), -((rk2+n)-mod(rk2+n,A.p))÷A.p
            ...
            tmpΛ[:,ik1,ik2,:,ip1,ip2,ipk1,m+4,n+1+3A.q] = view(A.vec,:,:,rk1+1,ik1,ik2)' * ΛΨ * view(A.vec,:,:,rp1+1,ip1,ip2)
        end
